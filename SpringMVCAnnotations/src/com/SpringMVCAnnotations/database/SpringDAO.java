package com.SpringMVCAnnotations.database;
...
public class SpringDAO {
	...
	public List<LoginForm> getUserList() throws SQLException, ParseException{
		PreparedStatement ps4 =con.prepareStatement("select * from user_details");
		ResultSet rs1 = ps4.executeQuery();
		List<LoginForm> userList = new ArrayList<>();
		...
		while(rs1.next()) {
			...
			obj.setPhoneNumber(phnNo);
			obj.setEmail(emailId);
			obj.setDate(dateSt);
			if(diff==0){
				obj.setDiffDate("Today");
							
						}
			else if(diff==1) {
				obj.setDiffDate("Yesterday");
				
			}
			else if(diff>1) {
				obj.setDiffDate(diff+" days ago");
